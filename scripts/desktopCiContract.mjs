...
const JOBS = {
  "macos-product": {
    ...
    commands: [
      ...
      "pnpm build",
      "pnpm broker:debug",
      "cargo fmt --manifest-path session-broker/Cargo.toml -- --check",
      "cargo clippy --manifest-path session-broker/Cargo.toml --all-targets --locked -- -D warnings",
      "cargo test --manifest-path session-broker/Cargo.toml --all-targets --locked",
