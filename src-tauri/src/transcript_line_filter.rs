...
//!
//! Codex records contain large tool and telemetry payloads that can never become conversation
...
//! and only lets user/assistant message records reach the full projection parser.
...
pub(crate) fn compact_record_relevance(source: TranscriptSource, line: &str) -> Option<bool> {
    ...
    match source {
        TranscriptSource::Codex => {
            ...
            let nested_at = ["\"content\":", "\"arguments\":", "\"input\":"]
                .into_iter()
            ...
            if payload.contains("\"role\":\"user\"") || payload.contains("\"role\":\"assistant\"") {
                return Some(true);
            }
            None
        }
...
mod tests {
    ...
    #[test]
    fn codex_only_passes_conversation_messages() {
        assert!(is_relevant(
            TranscriptSource::Codex,
