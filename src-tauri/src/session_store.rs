...
use crate::session_runtime::RuntimeError;
...
impl SessionStore {
    ...
    pub(crate) fn record(&self, session: &StoredSession) -> Result<(), RuntimeError> {
    ...
    pub(crate) fn append_output(&self, session_id: &str, bytes: &[u8]) {
        let Some(root) = self.root.as_deref() else {
            return;
        };
        if bytes.is_empty() {
    ...
    pub(crate) fn output(&self, session_id: &str) -> Vec<u8> {
        let Some(root) = self.root.as_deref() else {
            return Vec::new();
        };
